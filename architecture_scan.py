...
def find_potential_dead_code(files: list[Path]) -> list[dict[str, Any]]:
    """Find classes/functions that might be unused in same package."""
    # This is a simplified check — full cross-module requires full graph
    results = []
    for f in files:
        tree = parse_file(f)
        if not tree:
...
def check_test_coverage(api_files: list[Path], test_files: list[Path]) -> dict[str, Any]:
    api_mods = {f.stem for f in api_files}
    tested_mods = set()
...
def main() -> None:
    ...
    # 6. Dead code
    print("\n💀 6. POTENTIALLY UNUSED FUNCTIONS (same file)")
    dead = find_potential_dead_code(all_src)
    print(f"   Found {len(dead)} functions with zero in-file references")
    for d in dead[:8]:
