    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <h3 className="font-semibold text-sm">脚本段落</h3>
          <span className="text-xs text-muted-foreground">
            {segments.length} 段 · 预计{" "}
            {segments.reduce((sum, s) => sum + (s.duration || 0), 0)} 秒
          </span>
        </div>
        {onGenerate && (
          <Button
                ...
                {isExpanded && (
                  <CardContent className="pt-0 pb-3 px-3 space-y-3">
                    ...
                    <div>
                      ...
                      <Textarea
                        value={segment.speakerNote || ""}
                        onChange={(e) =>
                          updateSegment(segment.id, {
                            speakerNote: e.target.value,
                          })
                        }
                        placeholder="口播时的注意事项、语气、表情提示..."
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(Object.keys(SEGMENT_TYPES) as ScriptSegment["type"][]).map(
          (type) => {
            const config = SEGMENT_TYPES[type]
            const Icon = config.icon
            return (
      ...
      {segments.length === 0 && (
        <div className="text-center py-8 text-muted-foreground border rounded-lg border-dashed">
          ...
          <p className="text-xs mt-1">点击上方按钮添加段落或使用 AI 生成</p>
        </div>
      )}
    </div>
  )
