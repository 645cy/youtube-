...
class PathRecommender:
    ...
    @classmethod
    def from_json(cls, json_path: str) -> PathRecommender:
        """从 JSON 文件加载变现路径数据."""
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        paths = []
        for item in data:
            paths.append(MonetizationPath(**item))
...
def load_default_paths() -> list[MonetizationPath]:
    ...
    return [MonetizationPath(**p) for p in raw_paths]
